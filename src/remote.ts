...
type ActionType = "DEEP_LINK" | "DEEP_LINK";

export type NativeApps = "org.tizen.browser";
...
export class SamsungTvRemote {
  ...
  constructor(options: SamsungTvRemoteOptions) {
    ...
    if (Object.prototype.hasOwnProperty.call(apps, this.options.name)) {
      ...
      this.logger.warn(
        "No token found:",
        "app is not registered yet and will need to be authorized on TV"
      );
    }
   ...
   *
   * @async
   * @param {string} appID The application ID to be launched
   * @param {string} metaTag The meta tag to be sent
   * @param {string} action_type The action type to be sent
   * @returns {Promise<void>} A void promise
   */
  public async launchApp(
    appId: string,
    metaTag: string = "",
    action_type: ActionType = "DEEP_LINK"
  ): Promise<void> {
    if (appId) {
      const command = JSON.stringify({
      ...
      return new Promise((resolve) => {
        if (ws) {
          this.logger.log("Sending key:", appId);
   ...
   *
   * @async
   * @returns {Promise<App[]>} A App[] promise
   */
  public async listApps(): Promise<App[]> {
    const command = JSON.stringify({
      method: "ms.channel.emit",
      params: {
